...
test("POST '/api/v1/migrations'", async () => {
	...
	const wereDryRunMigrationExecutedAlive = compareMigrationsLists(
		getMigrations,
		migExecuted1,
	);
...
function compareMigrationsLists(list1, list2) {
	...
	for (const migrationIdx in list1) {
		...
		let uniqueKeys = Object.keys(migObj1).concat(Object.keys(migObj2));
		...
		if (
			uniqueKeys.length != Object.keys(migObj1).length ||
			uniqueKeys.length != Object.keys(migObj2).length
		)
			return false;

		// Objects has same Values? ---
		for (const key of uniqueKeys) {
			if (migObj1[key] != migObj2[key]) return false;
		}
	}
	return true;
}
